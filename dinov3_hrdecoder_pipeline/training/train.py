#!/usr/bin/env python3
"""
Train DINOv3 + HRDecoder Segmentation Model
=============================================
Usage:
    cd <bundle root>
    python -m dinov3_hrdecoder_pipeline.training.train
    python -m dinov3_hrdecoder_pipeline.training.train --gpus 2
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train DINOv3+HRDecoder")
    ...
    print("=" * 70)
    print("  DINOv3 + HRDecoder Training Pipeline")
    ...
    # Checkpoint: best by mIoU
    miou_ckpt = ModelCheckpoint(
        dirpath=ckpt_dir,
        filename=f"dinov3_hrdecoder_{decoder}_miou={{val/mIoU:.4f}}_acc={{val/overall_accuracy:.4f}}",
        monitor="val/mIoU",
        mode="max",
        save_top_k=1,
        save_last=True,
    ...
    # Checkpoint: best by overall accuracy
    ...
    # Report best checkpoints/scores
    try:
        print(f"  Best mIoU model: {miou_ckpt.best_model_path}")
        print(f"  Best mIoU:       {miou_ckpt.best_model_score:.4f}")
    except Exception:
        pass
    try:
        print(f"  Best accuracy model: {acc_ckpt.best_model_path}")
        print(f"  Best accuracy:       {acc_ckpt.best_model_score:.4f}")
    except Exception:
        pass
    print(f"{'='*70}\n")
