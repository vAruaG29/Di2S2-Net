...
Run from the bundle root (where configs/ paths resolve), on the server:

    conda activate svamitva2
    python analyze_dataset_stats.py
    # options:
    python analyze_dataset_stats.py --config dinov3_hrdecoder_pipeline/configs/train_full.yaml
    python analyze_dataset_stats.py --no-pixels        # tile/label counts only (fast)
    python analyze_dataset_stats.py --csv stats.csv    # also dump a CSV
...
PIPE_ROOT = Path(__file__).resolve().parent / "dinov3_hrdecoder_pipeline"
...
def main():
    ...
    # paths relative to this script's bundle (so it "just works" on the server).
    if not os.path.isdir(labels_dir):
        local = Path(__file__).resolve().parent / "masks"
        if local.is_dir():
            labels_dir = str(local)
    if not os.path.exists(tile_index):
        local_ti = Path(__file__).resolve().parent / "tiles" / "tile_index.csv"
        if local_ti.exists():
